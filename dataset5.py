...
# train_dataset = ImageFolder(text=root + 'images_train73.txt', transform=doTransform)
# valid_dataset = ImageFolder(text=root + 'images_valid73.txt', transform=doTransform)
# 75/25比例
...
valid_dataset = ImageFolder(text=root + 'images_valid82.txt', transform=doTransform)  # test: 12

# test_dataset = ImageFolder(text=root + 'images_test8.txt', transform=image_transforms['test'])  # test: 8
test_dataset = ImageFolder(text=root + 'images_test8.txt', transform=doTransform)  # test: 8
...
# 进行一次封装，将数据和标签封装成数据迭代器，这样才方便模型训练的时候一个batch一个batch地进行，这就要用到torch.utils.data.DataLoader接口
train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True)
# test_loader = DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True)
valid_loader = DataLoader(dataset=valid_dataset, batch_size=valid_size, shuffle=True, num_workers=workers, pin_memory=True)
test_loader = DataLoader(dataset=test_dataset, batch_size=test_size, shuffle=True, num_workers=workers, pin_memory=True)
show_loader = DataLoader(dataset=show_dataset, batch_size=show_size, shuffle=True, num_workers=workers, pin_memory=True)

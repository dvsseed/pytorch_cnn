...
def evaluate1():
    ...
    torch.save(model, 'cnn_shufflenetv2.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_shufflenetv2.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_shufflenetv2.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_shufflenetv2.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_shufflenetv2.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-57Layer-ShuffleNetv2')

    f = open('logs_shufflenetv2.txt', 'w')
    ftest = open('tests_shufflenetv2.txt', 'w')
    ...
    model = shufflenet_v2_x1_0()  # ShuffleNetv2
    model = model.to(device)
    if torch.cuda.device_count() > 1:
    ...
    if os.path.isfile('cnn_shufflenetv2.pth'):
        torch.save(model, 'cnn1_shufflenetv2.pth')
    else:
        torch.save(model, 'cnn_shufflenetv2.pth')
    if os.path.isfile('cnn_model_shufflenetv2.pth'):
        torch.save(model.state_dict(), 'cnn_model1_shufflenetv2.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_shufflenetv2.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_shufflenetv2.json')
    writer.close()

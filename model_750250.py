...
class ConvNet(nn.Module):
    ...
    def __init__(self, num_classes=4):  # 定義 model 中需要的參數，weight、bias 等
        ...
        self.layer1 = nn.Sequential(  # input shape (1, 256, 256)
            nn.Conv2d(in_channels=1, out_channels=16, kernel_size=3, stride=1, padding=1),  # output shape 16, 256, 256), padding=(kernel_size-1)/2 当 stride=1
        ...
        self.fc_layers = nn.Sequential(
            nn.Linear(in_features=32 * 32 * 64, out_features=512),
            # nn.ReLU(),
            nn.ReLU(True),
            # nn.LeakyReLU(),
            # nn.Dropout(0.2),

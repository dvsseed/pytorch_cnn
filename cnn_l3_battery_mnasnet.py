...
def evaluate1():
    ...
    torch.save(model, 'cnn_mnasnet.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_mnasnet.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_mnasnet.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_mnasnet.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_mnasnet.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-53Layer-MNASNet')

    f = open('logs_mnasnet.txt', 'w')
    ftest = open('tests_mnasnet.txt', 'w')
    ...
    if os.path.isfile('cnn_mnasnet.pth'):
        torch.save(model, 'cnn1_mnasnet.pth')
    else:
        torch.save(model, 'cnn_mnasnet.pth')
    if os.path.isfile('cnn_model_mnasnet.pth'):
        torch.save(model.state_dict(), 'cnn_model1_mnasnet.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_mnasnet.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_mnasnet.json')
    writer.close()

...
# from resnet import resnet50  # 五十層, Resnet
# from resnet import resnext50_32x4d  # 54層, ResNeXt
from resnet import wide_resnet50_2  # 54層, Wide ResNet
...
def evaluate1():
    ...
    torch.save(model, 'cnn_wide_resnet50_2.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_wide_resnet50_2.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_wide_resnet50_2.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_wide_resnet50_2.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_wide_resnet50_2.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-54Layer-WideResNet')

    f = open('logs_wide_resnet50_2.txt', 'w')
    ftest = open('tests_wide_resnet50_2.txt', 'w')
    ...
    model = wide_resnet50_2()  # Wide ResNet
    model = model.to(device)
    if torch.cuda.device_count() > 1:
    ...
    if os.path.isfile('cnn_wide_resnet50_2.pth'):
        torch.save(model, 'cnn1_wide_resnet50_2.pth')
    else:
        torch.save(model, 'cnn_wide_resnet50_2.pth')
    if os.path.isfile('cnn_model_wide_resnet50_2.pth'):
        torch.save(model.state_dict(), 'cnn_model1_wide_resnet50_2.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_wide_resnet50_2.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_wide_resnet50_2.json')
    writer.close()

...
# from googlenet import googlenet  # 115層, GoogLeNet
# from shufflenetv2 import shufflenet_v2_x1_0  # 57層, ShuffleNet v2
from mobilenet import mobilenet_v2  # 53層, MobileNet v2
...
def evaluate1():
    ...
    torch.save(model, 'cnn_mobilenet.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_mobilenet.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_mobilenet.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_mobilenet.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_mobilenet.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-53Layer-MobileNetv2')

    f = open('logs_mobilenet.txt', 'w')
    ftest = open('tests_mobilenet.txt', 'w')
    ...
    model = mobilenet_v2()  # MobileNet v2
    model = model.to(device)
    if torch.cuda.device_count() > 1:
    ...
    if os.path.isfile('cnn_mobilenet.pth'):
        torch.save(model, 'cnn1_mobilenet.pth')
    else:
        torch.save(model, 'cnn_mobilenet.pth')
    if os.path.isfile('cnn_model_mobilenet.pth'):
        torch.save(model.state_dict(), 'cnn_model1_mobilenet.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_mobilenet.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_mobilenet.json')
    writer.close()

...
from lenet import LeNet   # 五層, LeNet
...
def evaluate1():
    ...
    torch.save(model, 'cnn_lenet.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_lenet.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_lenet.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_lenet.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_lenet.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-5Layer-LeNet')

    f = open('logs_lenet.txt', 'w')
    ftest = open('tests_lenet.txt', 'w')
    # fmodel = open('lenet_model.txt', 'w')
    ...
    if os.path.isfile('cnn_lenet.pth'):
        torch.save(model, 'cnn1_lenet.pth')
    else:
        torch.save(model, 'cnn_lenet.pth')
    if os.path.isfile('cnn_model_lenet.pth'):
        torch.save(model.state_dict(), 'cnn_model1_lenet.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_lenet.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_lenet.json')
    writer.close()

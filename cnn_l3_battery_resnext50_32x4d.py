...
def evaluate1():
    ...
    torch.save(model, 'cnn_resnext50_32x4d.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_resnext50_32x4d.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_resnext50_32x4d.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_resnext50_32x4d.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_resnext50_32x4d.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-54Layer-ResNeXt')

    f = open('logs_resnext50_32x4d.txt', 'w')
    ftest = open('tests_resnext50_32x4d.txt', 'w')
    ...
    if os.path.isfile('cnn_resnext50_32x4d.pth'):
        torch.save(model, 'cnn1_resnext50_32x4d.pth')
    else:
        torch.save(model, 'cnn_resnext50_32x4d.pth')
    if os.path.isfile('cnn_model_resnext50_32x4d.pth'):
        torch.save(model.state_dict(), 'cnn_model1_resnext50_32x4d.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_resnext50_32x4d.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_resnext50_32x4d.json')
    writer.close()

...
def evaluate1():
    ...
    torch.save(model, 'cnn_densenet161.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_densenet161.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_densenet161.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_densenet161.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_densenet161.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-161Layer-DenseNet161')

    f = open('logs_densenet161.txt', 'w')
    ftest = open('tests_densenet161.txt', 'w')
    ...
    if os.path.isfile('cnn_densenet161.pth'):
        torch.save(model, 'cnn1_densenet161.pth')
    else:
        torch.save(model, 'cnn_densenet161.pth')
    if os.path.isfile('cnn_model_densenet161.pth'):
        torch.save(model.state_dict(), 'cnn_model1_densenet161.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_densenet161.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_densenet161.json')
    writer.close()

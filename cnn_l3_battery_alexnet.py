...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_alexnet.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_alexnet.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_alexnet.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_alexnet.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-8Layer-Alexnet')

    f = open('logs_alexnet.txt', 'w')
    ftest = open('tests_alexnet.txt', 'w')
    ...
    if os.path.isfile('cnn_alexnet.pth'):
        torch.save(model, 'cnn1_alexnet.pth')
    else:
        torch.save(model, 'cnn_alexnet.pth')
    if os.path.isfile('cnn_model_alexnet.pth'):
        torch.save(model.state_dict(), 'cnn_model1_alexnet.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_alexnet.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_alexnet.json')
    writer.close()

...
from resnet import resnet50   # 十八層, Resnet
...
def evaluate1():
    ...
    torch.save(model, 'cnn_resnet50.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_resnet50.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_resnet50.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_resnet50.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_resnet50.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-50Layer-Resnet50')

    f = open('logs_resnet50.txt', 'w')
    ftest = open('tests_resnet50.txt', 'w')
    ...
    if os.path.isfile('cnn_resnet50.pth'):
        torch.save(model, 'cnn1_resnet50.pth')
    else:
        torch.save(model, 'cnn_resnet50.pth')
    if os.path.isfile('cnn_model_resnet50.pth'):
        torch.save(model.state_dict(), 'cnn_model1_resnet50.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_resnet50.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_resnet50.json')
    writer.close()

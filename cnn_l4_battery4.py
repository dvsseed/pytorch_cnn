...
def evaluate():
    ...
    with torch.no_grad():
        ...
        # 验证的循环中，写入预测的准确度
        # 每10个batch画个点用于accu曲线
        # if i % 10 == 0:
        vepoch = 1
        # niter = vepoch * len(valid_loader) + i
        # 验证的循环中，写入预测的准确度
        writer.add_scalar('Validate/Accu', 100. * (vcorrect / vtotal), vepoch)
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    # 建立實體。資料存放在：'logs/'
    writer = SummaryWriter(logdir='./logs/', comment='CNN')
    ...
    writer.close()
    # 執行 Anaconda Prompt(Anaconda3)
    # (base) D:\pytorch_code>tensorboard --logdir logs

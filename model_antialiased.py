...
class ConvNet(nn.Module):
    ...
    def __init__(self, num_classes=4):  # 定義 model 中需要的參數，weight、bias 等
        ...
        # 初始化卷积层
        # self.layer1 = nn.Sequential(  # input shape (3, 256, 256)
        #    nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=1, padding=1),  # output shape 16, 256, 256), padding=(kernel_size-1)/2 当 stride=1
        ...
        self.layer2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=1, padding=0),
            Downsample(channels=8, filt_size=3, stride=2)
        )
        ...
        # self.fc_drop = nn.Dropout(p=0.2, inplace=False)

...
# from densenet import DenseNet   # 121層, DenseNet
# from inceptionv3 import InceptionV3   # 四十八層, InceptionV3
from inception import inception_v3   # 四十八層, InceptionV3
...
def train(doplot=False):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            # outputs, aux = model(inputs)  # to accept and unpack the returned tuple
...
def evaluate1():
    ...
    torch.save(model, 'cnn_inception.pth')
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        for i, (vinputs, vlabels) in enumerate(valid_loader):
            ...
            # voutputs, vaux = model(vinputs)  # to accept and unpack the returned tuple
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_inception.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_inception.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_inception.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_inception.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
    ...
    with torch.no_grad():
        ...
        for i, (tinputs, tlabels) in enumerate(test_loader):
            ...
            # toutputs, taux = model(tinputs)  # to accept and unpack the returned tuple
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-48Layer-InceptionV3')

    f = open('logs_inception.txt', 'w')
    ftest = open('tests_inception.txt', 'w')
    ...
    if os.path.isfile('cnn_inception.pth'):
        torch.save(model, 'cnn1_inception.pth')
    else:
        torch.save(model, 'cnn_inception.pth')
    if os.path.isfile('cnn_model_inception.pth'):
        torch.save(model.state_dict(), 'cnn_model1_inception.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_inception.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_inception.json')
    writer.close()

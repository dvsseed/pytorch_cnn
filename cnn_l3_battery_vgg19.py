...
def evaluate1():
    ...
    torch.save(model, 'cnn_vgg19.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_vgg19.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_vgg19.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_vgg19.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_vgg19.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-19Layer-VGG19')

    f = open('logs_vgg19.txt', 'w')
    ftest = open('tests_vgg19.txt', 'w')
    ...
    if os.path.isfile('cnn_vgg19.pth'):
        torch.save(model, 'cnn1_vgg19.pth')
    else:
        torch.save(model, 'cnn_vgg19.pth')
    if os.path.isfile('cnn_model_vgg19.pth'):
        torch.save(model.state_dict(), 'cnn_model1_vgg19.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_vgg19.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_vgg19.json')
    writer.close()

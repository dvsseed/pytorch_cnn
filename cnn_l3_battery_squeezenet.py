...
# from squeezenet0 import SqueezeNet   # 十層, SqueezeNet
from squeezenet import squeezenet1_0   # 十層, SqueezeNet
...
def evaluate1():
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in enumerate(valid_loader):
            ...
            # Tensor of shape 4, with confidence scores over Imagenet's 4 classes
            # print(outputs[0])
            # The output has unnormalized scores. To get probabilities, you can run a softmax on it.
            # print(torch.nn.functional.softmax(outputs[0], dim=0))
            # output0 = torch.nn.functional.softmax(outputs[0], dim=0)

            loss = loss_function(outputs, targets)
            # loss = loss_function(output0, targets)
            ...
            # pre_mask = torch.zeros(output0.size()).scatter_(1, predicted.cpu().view(-1, 1), 1.)
            predict_num += pre_mask.sum(0)
            tar_mask = torch.zeros(outputs.size()).scatter_(1, targets.cpu().view(-1, 1), 1.)
            # tar_mask = torch.zeros(output0.size()).scatter_(1, targets.cpu().view(-1, 1), 1.)
    ...
    torch.save(model, 'cnn_squeezenet.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_squeezenet.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_squeezenet.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_squeezenet.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_squeezenet.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-10Layer-SqueezeNet')

    f = open('logs_squeezenet.txt', 'w')
    ftest = open('tests_squeezenet.txt', 'w')
    ...
    model = squeezenet1_0()  # SqueezeNet
    model = model.to(device)
    if torch.cuda.device_count() > 1:
    ...
    if os.path.isfile('cnn_squeezenet.pth'):
        torch.save(model, 'cnn1_squeezenet.pth')
    else:
        torch.save(model, 'cnn_squeezenet.pth')
    if os.path.isfile('cnn_model_squeezenet.pth'):
        torch.save(model.state_dict(), 'cnn_model1_squeezenet.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_squeezenet.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_squeezenet.json')
    writer.close()

...
def evaluate1():
    ...
    torch.save(model, 'cnn_vgg16.pth')
...
def evaluate():
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_vgg16.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_vgg16.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_vgg16.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_vgg16.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-16Layer-VGG16')

    f = open('logs_vgg16.txt', 'w')
    ftest = open('tests_vgg16.txt', 'w')
    ...
    if os.path.isfile('cnn_vgg16.pth'):
        torch.save(model, 'cnn1_vgg16.pth')
    else:
        torch.save(model, 'cnn_vgg16.pth')
    if os.path.isfile('cnn_model_vgg16.pth'):
        torch.save(model.state_dict(), 'cnn_model1_vgg16.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_vgg16.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_vgg16.json')
    writer.close()

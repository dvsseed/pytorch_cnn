...
from resnet import resnet18   # 十八層, Resnet
...
def train(doplot=False):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            d_c = diskusage()
            print('Train Epoch: [{0:03}/{1:03}], Step [{2:02}/{3:02}], Loss: {4:.4f}, Accuracy: ({5:08.4f}%), Elapsed time: {6:06.4f} seconds, CPU: {7:02.2f}%, Memory: {8:02.2f}%, DiskIO: [{9:02.4f}%, {10:02.4f}%]'.format(
            ...
            f.write('Train Epoch: [{0:03}/{1:03}], Step [{2:02}/{3:02}], Loss: {4:.4f}, Accuracy: ({5:08.4f}%), Elapsed time: {6:06.4f} seconds, CPU: {7:02.2f}%, Memory: {8:02.2f}%, DiskIO: [{9:02.4f}%, {10:02.4f}%]\n'.format(
                    epoch + 1, num_epochs, i + 1, total_step, loss.item(), accuracy,
                    time.time() - tStart, psutil.cpu_percent(), psutil.virtual_memory().percent, d_c[0], d_c[1]))
...
def evaluate1():
    ...
    torch.save(model, 'cnn_resnet18.pth')
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        vloss /= vtotal
        d_c = diskusage()
        print('Validate set: Average Loss: {0:.4f}, Accuracy: [{1:02}/{2:02}] ({3:08.4f}%), Elapsed time: {4:06.4f} seconds, CPU: {5:02.2f}%, Memory: {6:02.2f}%, DiskIO: [{7:02.4f}%, {8:02.4f}%]'.format(
              vloss, vcorrect, vtotal, 100. * (vcorrect / vtotal), time.time() - eStart, psutil.cpu_percent(),
              psutil.virtual_memory().percent, d_c[0], d_c[1]))
        f.write("=" * 60)
        f.write('\n')
        f.write('Validate set: Average Loss: {0:.4f}, Accuracy: [{1:02}/{2:02}] ({3:08.4f}%), Elapsed time: {4:06.4f} seconds, CPU: {5:02.2f}%, Memory: {6:02.2f}%, DiskIO: [{7:02.4f}%, {8:02.4f}%]\n'.format(
                vloss, vcorrect, vtotal, 100. * (vcorrect / vtotal), time.time() - eStart, psutil.cpu_percent(),
                psutil.virtual_memory().percent, d_c[0], d_c[1]))
    ...
    if (100. * (vcorrect / vtotal)) > 10:  # 99:
        ...
        torch.save(model, 'cnn_resnet18.pth')
        # if torch.cuda.is_available():
        #    torch.save(model.module.state_dict(), 'cnn_model_resnet18.pth')
        # else:
        #    torch.save(model.state_dict(), 'cnn_model_resnet18.pth')
...
def test(doplot=False):
    ...
    # else:
    model = torch.load('cnn_resnet18.pth')  # , map_location='cpu')  # cnn_model.pth, evaluate.pth
    ...
    with torch.no_grad():
        ...
        for i, (tinputs, tlabels) in enumerate(test_loader):
            ...
            # testing the images
            ftest.write('index: {0:4}, label: {1:1}, predict: {2:1}, accuracy: {3:08.4f}%\n'.format(i + 1, tlabels[0], tpredicted[0], taccuracy))

        # print("Actual:", tlbls[:test_size], ">> Predicted:", tpreds[:test_size])

        tloss /= ttotal
        d_c = diskusage()
        print('Test set: Average Loss: {0:.4f}, Accuracy: [{1:02}/{2:02}] ({3:08.4f}%), Elapsed time: {4:06.4f} seconds, CPU: {5:02.2f}%, Memory: {6:02.2f}%, DiskIO: [{7:02.4f}%, {8:02.4f}%]'.format(
              tloss, tcorrect, ttotal, taccuracy, time.time() - tStart, psutil.cpu_percent(),
              psutil.virtual_memory().percent, d_c[0], d_c[1]))
        f.write('Test set: Average Loss: {0:.4f}, Accuracy: [{1:02}/{2:02}] ({3:08.4f}%), Elapsed time: {4:06.4f} seconds, CPU: {5:02.2f}%, Memory: {6:02.2f}%, DiskIO: [{7:02.4f}%, {8:02.4f}%]\n'.format(
                tloss, tcorrect, ttotal, taccuracy, time.time() - tStart, psutil.cpu_percent(),
                psutil.virtual_memory().percent, d_c[0], d_c[1]))
...
if __name__ == '__main__':
    ...
    # logger = Logger('./logs')
    writer = SummaryWriter(logdir='./logs/', comment='CNN-18Layer-Resnet18')

    f = open('logs_resnet18.txt', 'w')
    ftest = open('tests_resnet18.txt', 'w')
    ...
    if os.path.isfile('cnn_resnet18.pth'):
        torch.save(model, 'cnn1_resnet18.pth')
    else:
        torch.save(model, 'cnn_resnet18.pth')
    if os.path.isfile('cnn_model_resnet18.pth'):
        torch.save(model.state_dict(), 'cnn_model1_resnet18.pth')  # 保存
    else:
        torch.save(model.state_dict(), 'cnn_model_resnet18.pth')
    ...
    writer.export_scalars_to_json('./all_scalars_resnet18.json')
    writer.close()
